...
def quilt_patch_horizontal(patch_left, patch_right, overlap_len):
    patch_left_row, patch_left_col = patch_left.shape
    patch_right_row, patch_right_col = patch_right.shape
    ...
    luminance_diff = np.square(overlap_left - overlap_right)
...
def quilt_patch_vertical(patch_top, patch_bottom, overlap_len):
    patch_top_row, patch_top_col = patch_top.shape
    patch_bottom_row, patch_bottom_col = patch_bottom.shape
    ...
    luminance_diff = np.square(overlap_top - overlap_bottom)
...
def texture_transfer_keep_pic_color(texture_luminance, target_pic_luminance, block_size, overlap_len, random):
    texture_luminance_blur = Image.fromarray(texture_luminance, 'L').filter(ImageFilter.GaussianBlur(radius=3))
    texture_luminance_blur = np.array(texture_luminance_blur)
    target_pic_luminance_blur = Image.fromarray(target_pic_luminance, 'L').filter(ImageFilter.GaussianBlur(radius=3))
    target_pic_luminance_blur = np.array(target_pic_luminance_blur)


    result_luminance = np.zeros((1, 1))
    for row in range(0, target_pic_luminance.shape[0], block_size - overlap_len):
       
        now_row_size = target_pic_luminance.shape[0] - row
        ...
        now_row_data = texture_luminance[patch_row : patch_row + now_row_size, patch_col: patch_col + block_size]

        for col in range(block_size - overlap_len, target_pic_luminance.shape[1], block_size - overlap_len):
            #print((row, col))
            now_col_size = target_pic_luminance.shape[1] - col
            ...
            now_row_data = quilt_patch_horizontal(now_row_data, texture_luminance[patch_row: patch_row + now_row_size, patch_col: patch_col + now_col_size], overlap_len)
        if(row == 0):
...
if __name__ == '__main__':
    ...
    texture_luminance = np.array(texture.convert('L'))
    target_pic = Image.open(args.pic_path)
    target_pic_luminance = np.array(target_pic.convert('L'))

    if(args.random!=0):
        args.random = (10-args.random)*100
    

    luminance = texture_transfer_keep_pic_color(texture_luminance, target_pic_luminance, args.patchsize, args.overlap_len, args.random)
    img_out = np.array(Image.open(args.pic_path).convert("YCbCr"))
    img_out[:luminance.shape[0], :luminance.shape[1], 0] = luminance
